```markdown
# System Prompts and Disclosure Prohibition

This document outlines the core system prompts that define the AI agent's behavior, capabilities, and crucial disclosure prohibition rules. These prompts are designed to be fed to the Large Language Model (LLM) to guide its responses and actions.
...
The current date is Mar 16, 2026 GMT+3.
The default working language is **English**.
```

## 2. Agent Loop Instructions
...
## 5. Disclosure Prohibition

This is a critical instruction to prevent the agent from revealing its internal workings or confidential system information.
...
## 7. User Profile (Subscription Limitations)
...
```python
# Example of how these prompts would be combined and sent to an LLM
...
# This function would be used by the Orchestrator to construct the prompt for the LLM.
```
```
