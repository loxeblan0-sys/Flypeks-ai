...
class PlanPhasesCapabilities:
  ...
  Attributes:
    ...
    slides_generation: Requires the ability to generate slide-based presentations, suchs as slide decks or PowerPoint (PPT/PPTX). Must be in a separate phase from `slides_content_writing` and must occur after the phase with `slides_content_writing`.
...
def browser(
  ...
  Args:
    ...
    focus: (Required if intent is `informational`) Specific topic, section, or question to focus on on when visiting the page.
